...
export class PromptCompilerService {
  ...
  constructor(config: ConfigService<Env, true>) {
    ...
    if (!this.client) {
      this.logger.log(
        "ANTHROPIC_API_KEY is not set — using the structured fallback compiler.",
      );
    }
  }

  async compile(spec: AgentSpec): Promise<string> {
    ...
    try {
      ...
      this.logger.error(
        `Prompt compilation failed, using the structured fallback: ${
...
export function renderSpec(spec: AgentSpec): string {
  ...
  const facts: string[] = [];
  if (s(spec.companyDescription))
    facts.push(`About ${company}: ${s(spec.companyDescription)}`);
  if (s(spec.businessHours))
    facts.push(`Business hours: ${s(spec.businessHours)}`);
  if (s(spec.contactDetails))
    facts.push(`Contact and location: ${s(spec.contactDetails)}`);
