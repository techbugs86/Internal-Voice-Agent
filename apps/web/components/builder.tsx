...
import Recaptcha, { type RecaptchaHandle } from "./auth/recaptcha";
...
export default function Builder() {
  ...
  async function submit() {
    if (!captchaToken) {
      setError("Tick the “I'm not a robot” box before generating.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/agents", {
        ...
        headers: {
          ...
          // The BFF route forwards this to the API, whose guard verifies it
          // with Google before a single Anthropic or Retell call is made.
      ...
      // Pull the new agent into the list below without a full reload.
      router.refresh();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Something went wrong.");
    } finally {
      setBusy(false);
      // Single-use token — spent whether or not the build succeeded.
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Agent name" hint="what it calls itself on the call" required>
            <input
          ...
          <div className="sm:col-span-2">
            <Field label="First line" hint="leave empty to let the caller speak first">
              <input
      ...
      <Panel step={4} title={result ? "Your agent is live" : "Build"}>
        {!result ? (
          <>
            {/* Building an agent spends real money on Anthropic and Retell, so
                this gets the same check as the sign-in form. */}
            <div className="mb-4">
            ...
            <button
              onClick={submit}
              disabled={busy || !ready || !captchaToken}
            ...
            {!ready && !busy && (
              <p className="mt-3 text-[13px] text-[var(--color-muted)]">
                Fill in the agent name, the company name, and what it does or the
                services it offers.
              </p>
            )}
            {ready && !captchaToken && !busy && (
              <p className="mt-3 text-[13px] text-[var(--color-muted)]">
                Tick the verification box above to enable the button.
              </p>
            )}
          ...
          <>
            ...
            <p className="mt-4 text-[13px] text-[var(--color-muted)]">
              Calls with this agent end automatically after{" "}
              {MAX_CALL_DURATION_MS / 60_000} minutes.
            </p>
